...
async def _find_outward_junction(
    ...
    """
    Walk along directed motorway/motorway_link edges to find the nearest real junction
    node (a branch point — see _node_has_impacted_road). Duplicated from
    routers/network.py to avoid circular import.
    ...
    *previous edge's* bearing (incremental heading), not to the fixed overall
    ...
    guard against), which handles single-segment digitisation noise at pass-through
    nodes.
