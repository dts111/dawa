...
def _build_interpolator(surface: TINSurface):
    ...
    Returns a callable f(x_array, y_array) -> z_array, NaN outside the
    surface's TRUE triangulated footprint (not just the convex hull of its
    points).

    Values are computed from an auto-Delaunay triangulation of the (deduped)
    point cloud, exactly as before. Boundary masking, however, uses a SEPARATE
    triangulation built from the surface's own recorded faces: relying on the
    Delaunay hull alone silently fills in concave notches / non-convex
    boundaries with phantom area that isn't part of the actual surface (e.g. an
    L-shaped or bent corridor would get "squared off"). If the real faces can't
    form a valid matplotlib triangulation (e.g. genuinely duplicate points
    feeding degenerate/overlapping triangles), boundary masking is skipped and
    behaviour falls back to convex-hull-only masking rather than failing.
    """
    ...
    xu, yu, zu = x[unique_idx], y[unique_idx], z[unique_idx]

    triang = mtri.Triangulation(xu, yu)
    interp = mtri.LinearTriInterpolator(triang, zu)

    boundary_finder = None
    try:
        boundary_triang = mtri.Triangulation(x, y, triangles=surface.faces)
        boundary_finder = boundary_triang.get_trifinder()
    ...
    def query(qx, qy):
        ...
        if boundary_finder is not None:
            qx_arr = np.atleast_1d(np.asarray(qx, dtype=float))
            qy_arr = np.atleast_1d(np.asarray(qy, dtype=float))
            outside_true_footprint = boundary_finder(qx_arr, qy_arr) < 0
            result = np.where(outside_true_footprint, np.nan, result)
