...
import ExcavationProfilePanel from './components/ExcavationProfilePanel'
...
export default function App() {
  ...
  const [excavationResult, setExcavationResult] = useState(null)   // independent of the cut/fill `results` workflow

  const handleParsed = (data) => {
    setParseResult(data)
    setResults(null)
    setExcavationResult(null)
    setTab('3d')
  }
  ...
  const reset = () => { setParseResult(null); setResults(null); setExcavationResult(null) }
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-4 py-6 space-y-6">
            ...
            <div className="mt-3 card overflow-hidden">
              ...
              <div className="p-4">
                {tab === '3d' && results.allMeshes && (
                  <CutFillMap3D meshes={results.allMeshes} sessionKey={parseResult.session_key} showOverlayControls={false} />
                )}
                {tab === 'cutfill' && (
                  <CutFillMap2D
        ...
        {/* ── Step 5: Maximum Excavation Profile ── */}
        {parseResult && parseResult.surfaces.length >= 2 && (
          <section>
            <StepBadge n={5} label="Maximum Excavation Profile" active />
            <div className="mt-3">
              <ExcavationProfilePanel
                sessionKey={parseResult.session_key}
                surfaces={parseResult.surfaces}
                onResult={setExcavationResult}
              />
            </div>
          </section>
        )}
  ...
  return (
    <div className="flex items-center gap-2">
      <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold
        ${active ? 'bg-brand-500 text-white' : 'bg-slate-200 text-slate-500'}`}>
        {n}
      </span>
      <span
        className={`text-sm font-semibold ${active ? 'text-ink' : 'text-slate-400'}`}
