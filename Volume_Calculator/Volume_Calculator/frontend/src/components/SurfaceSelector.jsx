  ...
  return (
    <div className="card p-6">
      ...
      <p className="text-sm text-slate-500 mb-5">
        Found <strong className="text-slate-700">{surfaces.length}</strong> TIN surface{surfaces.length !== 1 ? 's' : ''}.
      ...
      <div className="grid sm:grid-cols-2 gap-4 mb-4">
        <div>
          ...
          {surfaceMap[surface1] && (
            <SurfaceInfo surface={surfaceMap[surface1]} color="text-brand-600" />
          )}
        </div>
      ...
      <details className="mb-4">
        ...
        <div className="mt-3 pl-6 border-l border-slate-200">
      ...
      {/* Error */}
      {error && (
        <div className="mb-4 flex items-start gap-2 rounded-lg bg-red-50 border border-red-300 p-3 text-sm text-red-600">
...
function SurfaceInfo({ surface, color }) {
  return (
    <div className={`mt-2 text-xs space-y-0.5 ${color}`}>
      ...
      {surface.desc && <p className="text-slate-400 truncate" title={surface.desc}>{surface.desc}</p>}
    </div>
  )
}
