        ...
        <div className="relative">
          ...
          <div className="flex flex-wrap gap-x-5 gap-y-1 mt-3 px-2">
            {surfaceNames.map((name, i) => (
              <span key={name} className="flex items-center gap-1.5 text-xs text-slate-700">
            ...
            <span className="ml-auto text-xs text-slate-400">Scroll = zoom · Drag = pan</span>
          </div>
        </div>
      )}
    ...
    <div>
      ...
      <div className="flex gap-2">
        <div className="flex-1">
          <span className="text-xs text-slate-400">X (Easting)</span>
          <input
            type="number"
            step="any"
        ...
        <div className="flex-1">
          <span className="text-xs text-slate-400">Y (Northing)</span>
          <input
            type="number"
            step="any"
            className="select-input mt-0.5 text-sm"
...
function drawChart(profileData, svgEl, containerEl, zoomRef) {
  ...
  const axStyle = (sel) => sel.selectAll('text, line, path')
    .attr('stroke', '#64748b').attr('fill', '#64748b').attr('font-size', '11')
  ...
  svg.append('text').attr('x', margin.left + iW / 2).attr('y', 18)
    .attr('text-anchor', 'middle').attr('fill', '#1a1c1e').attr('font-size', '13').attr('font-weight', '600')
  ...
  g.append('g').attr('class', 'grid')
    ...
    .call(s => s.selectAll('.tick line').attr('stroke', '#d8dcda').attr('stroke-dasharray', '3,3'))
